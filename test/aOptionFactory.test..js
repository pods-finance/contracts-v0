...
describe('aOptionFactory', function () {
  ...
  it('Should create a new Option correctly and emit event', async function () {
    ...
    await expect(optionFactory.createBearingOption(...funcParameters)).to.emit(optionFactory, 'OptionCreated')
  })
  ...
  it('Should revert if calling createOption with block lower than currentBlock', async function () {
    ...
    await expect(optionFactory.createBearingEthOption(...funcParameters)).to.be.revertedWith('Expiration lower than current block')
  })
})
